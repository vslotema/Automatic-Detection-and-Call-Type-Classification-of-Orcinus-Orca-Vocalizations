...
import keras_applications.imagenet_utils
...
def DenseNet(blocks,
    ...
    input_shape = _obtain_input_shape(input_shape,
                                      default_size=224,
                                      min_size=32,
                                      data_format=keras.backend.image_data_format(),#keras_applications.backend.image_data_format(),
    ...
    x = keras.layers.Conv2D(64, 7, strides=2, use_bias=False, name='conv1/conv')(x)
    ...
    if include_top:
        ...
        x = keras.layers.Dense(classes, activation='sigmoid', name='fc1000')(x) #'softmax'
    else:
        if pooling == 'avg':
            x = keras.layers.GlobalAveragePooling2D(name='avg_pool')(x)
        elif pooling == 'max':
    ...
    if blocks == [6, 12, 24, 16]:
        model = keras.models.Model(inputs, x, name='densenet121')
    elif blocks == [6, 12, 32, 32]:
        model = keras.models.Model(inputs, x, name='densenet169')
    elif blocks == [6, 12, 48, 32]:
        model = keras.models.Model(inputs, x, name='densenet201')
    else:
        model = keras.models.Model(inputs, x, name='densenet')
    ...
    if weights == 'imagenet':
        if include_top:
            ...
            if blocks == [6, 12, 24, 16]:
                ...
                weights_path = keras.keras_utils.get_file(
                    'densenet169_weights_tf_dim_ordering_tf_kernels_notop.h5',
                    DENSENET169_WEIGHT_PATH_NO_TOP,
                    cache_subdir='models',
...
def preprocess_input(x, data_format=None, **kwargs):
    ...
    # Returns
        Preprocessed array.
    """
    return keras_applications.imagenet_utils.preprocess_input(x, data_format,
                                           mode='torch', **kwargs)
